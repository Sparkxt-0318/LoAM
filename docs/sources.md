...
| **Verra VM0042 — Improved Agricultural Land Management.** v2.2 greenlined, October 2025, 183 pp. — [PDF](https://verra.org/wp-content/uploads/2025/10/VM0042v2.2_greenlined.pdf); and the v2.0 final-draft-for-public-comment, December 2021, 173 pp. — [PDF](https://verra.org/wp-content/uploads/2021/12/VM0042_v2.0_FinalDraft_PublicComment.pdf) | The Phase 5 audit hook of D-038, now verified. **§8.6.4 Eq. 74** confirms `t0.667` ≈ **0.4307** (33.3rd percentile). **§8.2.1 item 11 Eq. (2)** `n ≥ (S(t_α+t_β)/MDD)²` **is the inverted audit**, and the methodology states "projects are not required to take this number of samples". Also supplies the ESM requirement (≥2 increments at re-sampling, ≥30 cm, von Haden 2020's R script accepted), 5-yearly remeasurement, ≥3 control sites and ≥1 per stratum, and the laboratory analytical-error definition (repeated analyses of the same sample — relevant to D-054). See D-057. | ✅ **read from the PDF**; ⚠️ 0.4307 is verified in **v2.2 only** — the v2.0 document held is a tracked-changes redline and does not contain it |
