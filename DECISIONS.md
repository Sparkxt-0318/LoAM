...
| D-059 | decided | ⚠️ **RETRACTED IN PART by D-060** — read from a superseded draft. `UNC = MDD ÷ Δ` and the case-N3 reading are void |
| D-060 | decided | **CORRECTION**: approved VM0042 uses one rule, `k = 0.4307`, in every version; `s²` identical across versions; VCS 4022's claim is **0.70× its own MDD**, not 3.19× |
| D-061 | decided | **THE THRESHOLD IS VERRA'S**: MR §2.4 has no cap but an eligibility bar at `UNC > 26.18%`; `k = 0.4307` is VCS program-wide; VCS 4022's implied 90% half-width is **119.7%** of its estimate. Sweep blocked at n = 1 |
| D-062 | decided | **RETRIEVAL LOG + THREE-WAY DISTINCTION**: disclosure gap ≠ access wall ≠ environment limitation. Authorised browser failed on *our* egress, not Verra's. `verra.org/robots.txt` disallows `/wp-content/uploads` — disclosed, standing rule adopted |
...
| 2026-08-13 | **D-060 — D-059 RETRACTED IN PART** | **The `s²` check was run first, per PI direction, and it falsified its own premise.** `docs/vm0042_s2_verification.md`. **The narrow answer is yes:** `s²` is the same object in v2.0 and v2.2 — same definition and units, same §8.6 subsection tree heading-for-heading, and the equations that build it are identical symbol-for-symbol (v2.0 Eqs. 54/55 ≡ v2.2 Eqs. 63/64, same Cochran 1977 Eq. 13.39 and Som 1995 Eq. 25.10 citations; the equation maps differ by a uniform +9). Model prediction variance is pooled with sampling variance in both. **But there is no Eq. 84 to compare against, because there is no Eq. 83.** The **approved** VM0042 v2.0 §8.6.4 (pp. 74–76) is a single **Equation (65)**, `UNC = (√s²/Δ̄ × 100) × t_{α=0.666}`, with the constant given verbatim as "*approximately **0.4307** at large sample sizes*" — **the same rule and the same constant as v2.2 Eq. 74.** Verbatim searches for `Pathway A`, `Case N1`, `Case N3` return **zero** hits across approved v2.0, v2.1, v2.2, the v2.2 greenlined text, the Corrections & Clarifications to v2.0, and the v3.0 consultation draft; `0.4307` appears in **every** one of them. **Provenance identified, and it is not fabrication:** D-059's quotations are accurate transcriptions of the **December 2021 final-draft-for-public-comment**, which genuinely carries the two-pathway/two-case §8.6.4 (Eqs. 83–86, threshold `A`, `t₀.₉₇₅`, exceedance 55%/70%, the Australian 60% footnote). **Verra removed that construction before approving v2.0 in May 2023.** The prior record flagged exactly this risk in its own words — "*The APPROVED v2.0 has not been obtained; §8.6.4 is read from the [draft]*" — and listed obtaining it as blocked; **this closes that item in the direction that invalidates the result.** *A public-comment draft is not evidence of the rule in force, even when it is the only copy to hand.* **What is void:** the 4.54× version-severity ratio (there is no ratio — `k = 0.4307` in every version); `UNC = MDD/Δ` as an identity; the case-N3 determination (VM0042 has no cases, so §8.2.8's QA1/QA3 test, itself correctly transcribed, selects nothing); and the `MIN{100%, …}` cap "detector" — **approved VM0042 has no cap**, so the planned cap sweep has nothing to detect unless one is found in *VCS Methodology Requirements* §2.4, which §8.6.4 defers to and which is unretrieved. **What survives:** the general identity **`Δ/MDD = k/((t_α+t_β)·UNC)`** — only `k` was wrong; the Eqs. 36/37 share factor (18.48% effective on gross ERRs); the four withheld/undisclosed inputs and the estimand mismatch; and the optionality finding, relocated to **§8.2.1.3 item 10**, verbatim "*However, projects are not required to take this number of samples*", with MDD at **§8.2.1.3 Equation (2)**, not the nonexistent "§9.3.1 Eq. 88". **VCS 4022 inverts, and hardens:** implied relative SE **72.79%** not 15.99%; `Δ/MDD` = **0.70** at 50% power and **0.49** at 80% — the claimed change is **below its own minimum detectable difference**, where D-059 reported it comfortably above at 3.19×. The sweep scope already contained the contradiction: its §2 threshold table says `UNC > 21.97%` ⟹ below floor, and 31.35% clears that line, while §3a said 3.19× above. **Net effect on Phase 5: the sweep gets cheaper and wider** — one rule, one constant, no version/case/pathway determination per project, and the VM0042 screen is a single line at **UNC > 21.97%** (50% power) / **15.37%** (80%). **Two new gating items, both cheap:** re-pull the VCS 4022 PD to confirm 31.35% is the Eq. 65 deduction for SOC (not re-verified here), and retrieve *VCS Methodology Requirements* §2.4 for a cap. No variance-table row written or promoted. |
| 2026-08-13 | D-061 | **THE SWEEP'S THRESHOLD IS VERRA'S OWN, AND THE SWEEP IS BLOCKED AT n = 1.** Both D-060 gating items worked, one closing and one not. **(1) MR §2.4 retrieved (v4.4, 4 Oct 2023, 88 pp.), and it carries more than the cap it was checked for.** *(a)* `k = 0.4307` is a **VCS program-wide** constant, not a VM0042 one: §2.4 defines `Discount factor = (Uncertainty / t_{α=10%}) × t_{α=66.6%}` with `1.6449` and `0.4307` verbatim and `Uncertainty` = the 90% CI half-width as a percentage of the mean, which reduces to `0.4307·σ_rel` — identical to VM0042 Eq. 65/74, and confirmed by Verra's own worked example (`32.9% / 1.6449 × 0.4307 = 8.6%`). **The inversion therefore generalises to every VCS methodology implementing §2.4, not just VM0042.** *(b)* **There is no cap** at program level either, so **the planned cap sweep is deleted, not deferred.** *(c)* **But there is an eligibility bar**, verbatim: "*Where the half-width of the two-sided 90 percent confidence interval exceeds **100 percent** of the reduction and removal estimate, **the project is not eligible for crediting**.*" That is `1.6449·SE > Δ` ⟺ `σ_rel > 60.79%` ⟺ **`UNC > 26.18%`** — **a single threshold set by Verra, with no power convention chosen by us**, which is strictly better than the 21.97% / 15.37% power-based lines and is what the sweep will run at. For orientation the bar sits at `Δ/MDD = 0.839`: **the VCS program explicitly permits crediting claims down to 84% of their own 50%-power minimum detectable difference.** Cross-checked against MR v4.0 (Sept 2019): it contains **neither** `0.4307` nor the bar, so both entered between 2019 and 2023. **(2) VCS 4022 against Verra's bar: implied 90% CI half-width = 119.7% of the ERR estimate, 19.7 points over.** *Interpretation stated, not asserted:* VM0042 v2.0 §8.6.4 points to "*the latest version of*" §2.4 but does not reproduce the bar, and the bar is a v4.4-era provision (Oct 2023) postdating VM0042 v2.0's approval (May 2023) and predating this project's validation (Dec 2024, VCS Standard v4.5) — so **either the bar binds through that pointer, or VM0042 does not implement a program requirement it explicitly cross-references. Both are findings; neither is an accusation**, and resolving which holds needs Verra's own reading, which we do not have. **(3) The 31.35% scope question was attempted and NOT confirmed.** The PD is not public outside the SPA registry; the validation report (publicly mirrored) calls the figure "*the final uncertainty deduction*" (CAR ID 60), singular and project-level, in Table 4-17 and §4.4.4 without disaggregation, while VM0042 §8.6.4 requires deductions "*estimated and applied separately for each ERR source*". **This splits the two findings by robustness, which is the useful part:** the §2.4 half-width comparison **does not depend** on the pool scope (both sides refer to the same ERR estimate) and is **robust**; the `Δ/MDD = 0.70` ratio **does** depend on it (MDD is defined on SOC stocks, §8.2.1.3 Eq. 2) and stays **conditional**. Lead with the eligibility comparison. **(4) THE SWEEP DID NOT RUN.** `registry.verra.org` returns a 2598-byte SPA shell (`text/html`, HTTP 200) on **every** route tried, GET and POST alike, including `/uiapi/resource/resourceSummary/search` and `/uiapi/asset/asset/search` with JSON bodies and browser headers — two further rounds (three HTTP calls) on top of D-056's. Targeted web search surfaced no other VM0042 project publishing a deduction. **Population reachable without a headless browser: n = 1. No distribution is reported and none was estimated.** Method, constant and threshold are all settled; only the sample is missing, and the headless-browser decision (sweep scope §8 item 5) is now the single blocker on the whole of Phase 5. No variance-table row written or promoted. |
| 2026-08-13 | D-062 | **THE ELIGIBILITY BAR WRITTEN UP AS ITS OWN RESULT; SWEEP ATTEMPTED UNDER AUTHORISATION AND STOPPED BY OUR OWN ENVIRONMENT.** `docs/vcs_eligibility_bar.md` — a **program-level** result, deliberately not filed inside a VM0042 document: *VCS Methodology Requirements* §2.4 bars crediting where the 90% CI half-width exceeds 100% of the ERR estimate, which is `σ_rel > 60.79%`, i.e. **`UNC > 26.18%`**, sitting at **`Δ/MDD = 0.839`** — **the VCS program permits crediting of claims down to 84% of their own 50%-power minimum detectable difference.** Not our threshold, not our power convention, not our variance components: Verra's rule inverted through Verra's own constant, confirmed by Verra's own worked example (`32.9% / 1.6449 × 0.4307 = 8.6%`). **`k = 0.4307` is stated as its own claim — a VCS program-wide constant, not a VM0042 one** — so the addressable surface is every VCS project publishing a §2.4 deduction, bounded below by §2.4's own 10% materiality floor. **The robustness split is kept prominent**: the half-width comparison does **not** need 31.35% to be SOC-specific (both sides are the same ERR estimate) and is **robust**; `Δ/MDD = 0.70` **does** (MDD is on SOC stocks) and is labelled **conditional** everywhere it appears, `projects.yaml` included. VCS 4022's framing preserved verbatim and explicitly marked not-to-be-sharpened: 119.7% implied half-width, 19.7 points past the bar; **either** the bar binds through §8.6.4's "latest version of" pointer **or** VM0042 does not implement a requirement it cross-references; **both are findings, neither is an accusation.** **Version-boundary caution logged as a standing rule** after a second near-miss (VM0042 draft vs approved, then MR v4.0 vs v4.4 — v4.0 has neither the constant nor the bar): *a draft is never evidence of a rule in force, and a deferred-to program document must be retrieved at its own version, because the two revise on independent schedules.* Both near-misses were caught by retrieving a primary source a prior session had recorded as unobtained; neither was caught by internal review, since both readings were internally consistent and wrong. **SWEEP: authorised, attempted, not completed — and not because of Verra.** `robots.txt` checked first, with one result each way: `registry.verra.org` serves **no** robots.txt (SPA catch-all, no parseable directive, target paths **not** disallowed), but **`verra.org` disallows `/wp-content/uploads`, which is where every primary document in this project's evidence base came from** — disclosed rather than argued away, with the REP's crawling scope noted, document content unaffected, and a **standing rule adopted: no automated or bulk retrieval under that path; any future bulk fetch is a PI decision.** The authorised browser then failed on **our** egress: Chromium returned `net::ERR_CONNECTION_RESET` for **every** host including `example.com`, across two rounds (default + explicit proxy; then `--proxy-server`, QUIC off, background networking off, cert errors ignored), because the container's proxy accepts only `CONNECT` tunnels — while `curl` reaches the same hosts from the same shell. **THE THREE-WAY DISTINCTION, never to be merged: disclosure gap (ACCU — 999 projects, zero sampling fields, complete-population evidence about the registry) ≠ access wall (Verra — public but client-rendered, evidence about auditability) ≠ environment limitation (ours — evidence about nothing but us).** The surviving joint finding is recorded beside the ACCU census: **the two largest soil-carbon registries publish design and uncertainty data in forms that cannot be systematically audited — complete-population evidence for one, access-wall evidence for the other** — and neither statement depends on anything LoAM computes. Population reachable in this environment **n = 1**; no distribution reported and none estimated. Constraints honoured throughout: 4–6 s between navigations, no parallelism, no credentials held or attempted, abort-on-login-wall, two rounds per method then stop, cap not raised. No variance-table row written or promoted. |
